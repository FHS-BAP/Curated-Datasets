...
Revised by Alvin Ang, Michael Cummings	
...
1)  vr_wkthru_ex10_1b_1488_v1 (Gen 2/Omni 1 workthru)
	vr_raceall_2011_a_1257_v1 (FHS race/ethnicity information)
	vr_educ_2018_a_1307_v1 (Harmonized education dataset for all cohorts)

2)  np_summary_through2024_19899 (NP Summary Score)

3)  demrev_2024 (Dementia Review)

4)  Commonly used outcomes (CHD, Stroke, AF, Diabetes and Cancer): 
	vr_soe_2023_a_1553 (Verified Sequence of Events (SOE) Cardiovascular Endpoints)
	vr_afcum_2023_a_1510 (verified Atrial Fibrillation/Flutter dataset)
	vr_diab_ex10_1b_1489_v1 (Verified Diabetes dataset for Gen 2/Omni 1)
	vr_cancer_2019_a_1162_v2 (Verified Cancer dataset)
	vr_survdth_2022_a_1537 (Verified Survival Status dataset)

5) 	Individual FHS exam questionnaires: 
	e_exam_ex01_1_0079_v4 (Gen 2 exam 1)
	e_exam_ex02_1_0080_v4 (Gen 2 exam 2)
	e_exam_ex03_1_0081_v2 (Gen 2 exam 3)
	e_exam_ex04_1_0082_v2 (Gen 2 exam 4)
	e_exam_ex05_1_0083_v2 (Gen 2 exam 5)
	e_exam_ex06_1_0084_v1 (Gen 2 exam 6)
	e_exam_ex07_1_0085_v2 (Gen 2 exam 7)
	e_exam_ex08_1_0005_v1 (Gen 2 exam 8)
	...
	e_exam_ex09_1b_0844_v1 (Gen 2/Omni 1 exam 9)
	e_exam_ex10_1b_1409_v1 (Gen 2/Omni 1 televisit/exam 10)
	vr_mmse_ex10_1b_1395_v1 (Verified Gen 2/Omni1 MMSE dataset)

6)  Neuropathology Data 
	neuropath_through2024_269 (FHS Brain Donor Neuropathology Data) 

*Provide the location of these datasets to import from before you run the SAS code. ;
libname in1 'C:\Users\mcum\Desktop\SAS_curated_files\curated_2\Input_files';
*Provide the location of the derived datasets to export to before you run the SAS code. ;
libname out1 'C:\Users\mcum\Desktop\SAS_curated_files\curated_2\Output_files';
...
data wt_1_1;
set in1.vr_wkthru_ex10_1b_1488_v1;
if idtype =1; 
...
retain framid; 
set in1.vr_raceall_2011_a_1257_v1; 
*Creating universal identifer variables - framid;
...
if findc(RACE_CODE,'R') then race = 6;
else if race_code in ("B", "BB", "BEB", "BHB", "EB", "EBB", "EBEB", "HHB") then race = 2;
else if race_code in ("A", "EA", "EAA", "EAEA", "HHA") then race = 3;
else if race_code in ("E", "EW", "W", "EEW", "EWE", "EWEW", "EWW", "WH", "EWHW", "H", "HH", "HHW", "HW", "WHHW", "HEW", "WHEW", "") then race = 1;
else if race_code in ("EPEP", "X") then race = 4;
else if race_code in ("N", "EN","HHN") then race = 5;
else race = 6;
*Creating derived variable - Ethnicity;
if race_code in ("EOHW", "EWHW", "HEW", "WHEW") then ethnicity = .; 
else if findc(RACE_CODE,'H') then ethnicity = 2; 
else ethnicity = 1;
...
keep framid race_code race ethnicity;
run; 
...
data diabetes_1; 
retain framid; 
set in1. vr_diab_ex10_1b_1489_v1; 
...
data backbone_17;
retain idtype id framid sex race_code race ethnicity; 
...
******************************************************************************************************************************************
Alcohol consumption for Gen 2/ Omni 1 
******************************************************************************************************************************************

/*Citation: Sun X, Ho JE, Gao H, Evangelou E, Yao C, Huan T, Hwang SJ, Courchesne P, Larson MG, Levy D, Ma J, Liu C. Associations of Alcohol Consumption with Cardiovascular Disease-Related Proteomic Biomarkers: The Framingham Heart Study.*/;

/*Create list with sex variable for alcohol consumption*/
data list_sex_17;
set backbone_17;
keep idtype id framid sex; 
run; 

/*Gen 2 exam 1 alcohol data extraction*/
data alcohol_1; 
set in1.e_exam_ex01_1_0079_v4;
  beerperweek1   = A111;
  wineperweek1   = A112;
  liquorperweek1 = A113;
/*Calculate number of drinks and alcohol consumption per week*/
  drinksperweek1 = sum(of beerperweek1 wineperweek1 liquorperweek1);
  gramsperday1 = sum(beerperweek1*13.9 , 15*wineperweek1,14*liquorperweek1)/7;
/*Create universal identifier - framid */
  framid = id + 80000;
keep framid beerperweek1 wineperweek1 liquorperweek1 drinksperweek1 gramsperday1;
run;


/*Gen 2 exam 2 alcohol data extraction*/
data alcohol_2; 
set in1.e_exam_ex02_1_0080_v4;
  beerperweek2   = B117;
  wineperweek2   = B118;
  liquorperweek2 = B119;
/*Calculate number of drinks and alcohol consumption per week*/
  drinksperweek2 = sum(of beerperweek2 wineperweek2 liquorperweek2);
  gramsperday2 = sum(beerperweek2*13.9 , 15*wineperweek2,14*liquorperweek2)/7;
/*Create universal identifier - framid */
  framid = id + 80000;
keep framid beerperweek2 wineperweek2 liquorperweek2 drinksperweek2 gramsperday2;
run;


/*Gen 2 exam 3 alcohol data extraction*/
data alcohol_3; 
set in1.e_exam_ex03_1_0081_v2;
  beerperweek3   = C83;
  wineperweek3   = C86;
  liquorperweek3 = C89;
/*Calculate number of drinks and alcohol consumption per week*/
  drinksperweek3 = sum(of beerperweek3 wineperweek3 liquorperweek3);
  gramsperday3 = sum(beerperweek3*13.9 , 15*wineperweek3,14*liquorperweek3)/7;
/*Create universal identifier - framid */
  framid = id + 80000;
keep framid beerperweek3 wineperweek3 liquorperweek3 drinksperweek3 gramsperday3;
run;


/*Gen 2 exam 4 alcohol data extraction*/
data alcohol_4; 
set in1.e_exam_ex04_1_0082_v2;
  beerperweek4   = D082;
  wineperweek4   = D085;
  liquorperweek4 = D088;
/*Calculate number of drinks and alcohol consumption per week*/
  drinksperweek4 = sum(of beerperweek4 wineperweek4 liquorperweek4);
  gramsperday4 = sum(beerperweek4*13.9 , 15*wineperweek4,14*liquorperweek4)/7;
/*Create universal identifier - framid */
  framid = id + 80000;
keep framid beerperweek4 wineperweek4 liquorperweek4 drinksperweek4 gramsperday4;
run;


/*Gen 2 exam 5 alcohol data extraction*/
data alcohol_5; 
set in1.e_exam_ex05_1_0083_v2;
  beerperweek5   = E310;
  wineperweek5   = E313;
  liquorperweek5 = E316;
/*Calculate number of drinks and alcohol consumption per week*/
  drinksperweek5 = sum(of beerperweek5 wineperweek5 liquorperweek5);
  gramsperday5 = sum(beerperweek5*13.9 , 15*wineperweek5,14*liquorperweek5)/7;
/*Create universal identifier - framid */
  framid = id + 80000;
keep framid beerperweek5 wineperweek5 liquorperweek5 drinksperweek5 gramsperday5;
run;


/*Gen 2 exam 6 alcohol data extraction*/
data alcohol_6; 
set in1.e_exam_ex06_1_0084_v1;
  beerperweek6  = F276;
  whiteperweek6 = F279;
  redperweek6   = F282;
  liquorperweek6= F285;
/*Calculate number of drinks and alcohol consumption per week*/
  drinksperweek6 = sum(of beerperweek6 whiteperweek6 redperweek6 liquorperweek6);
  gramsperday6 = sum(beerperweek6*13.9 , 15*whiteperweek6, 15*redperweek6,14*liquorperweek6)/7;
/*Create universal identifier - framid */
  framid = id + 80000;
keep framid beerperweek6 whiteperweek6 redperweek6 liquorperweek6 drinksperweek6 gramsperday6;
run;


/*Gen 2 exam 7 alcohol data extraction*/
data alcohol_7; 
set in1.e_exam_ex07_1_0085_v2;
  beerperweek7  = G104;
  whiteperweek7 = G107;
  redperweek7   = G110;
  liquorperweek7= G113;
/*Calculate number of drinks and alcohol consumption per week*/
  drinksperweek7 = sum(of beerperweek7 whiteperweek7 redperweek7 liquorperweek7);
  gramsperday7 = sum(beerperweek7*13.9 , 15*whiteperweek7, 15*redperweek7, 14*liquorperweek7)/7;
/*Create universal identifier - framid */
  framid = id + 80000;
  keep framid beerperweek7 whiteperweek7 redperweek7 liquorperweek7 drinksperweek7 gramsperday7;
run;


/*Gen 2 exam 8 alcohol data extraction*/
data alcohol_8; 
set in1.e_exam_ex08_1_0005_v1;
  beerperweek8  = H072;
  wineperweek8 = H075;
  liquorperweek8= H078;
/*Calculate number of drinks and alcohol consumption per week*/
  drinksperweek8 = sum(of beerperweek8 wineperweek8 liquorperweek8);
  gramsperday8 = sum(beerperweek8*13.9 , 15*wineperweek8, 14*liquorperweek8)/7;
/*Create universal identifier - framid */
  framid = id + 80000;
  keep framid beerperweek8 wineperweek8 liquorperweek8 drinksperweek8 gramsperday8;
run;


/*Omni 1 exam 1 (similar protocol to Gen 2 exam 5) alcohol data extraction*/
data alcohol_omni1_5; 
set in1.e_exam_ex01_7_0020;
  beerperweek5   = E310;
  wineperweek5   = E313;
  liquorperweek5 = E316;
/*Calculate number of drinks and alcohol consumption per week*/
...
/*Omni 1 exam 2 (similar protocol to Gen 2 exam 7) alcohol data extraction*/
data alcohol_omni1_7; 
...
/*Omni 1 exam 3 (similar protocol to Gen 2 exam 8) alcohol data extraction*/
data alcohol_omni1_8; 
...
/*Gen 2 Exam 9 Omni 1 exam 4 alcohol data extraction*/
data alcohol_9; 
set in1.e_exam_ex09_1b_0844_v1;
  beerperweek9  = J075;
  wineperweek9 = J077;
  liquorperweek9 = J079;
/*Calculate number of drinks and alcohol consumption per week*/
  drinksperweek9 = sum(of beerperweek9 wineperweek9 liquorperweek9);
  gramsperday9 = sum(beerperweek9*13.9 , 15*wineperweek9, 14*liquorperweek9)/7;
/*Create universal identifier - framid */
  if idtype = 1 then framid = id + 80000;
  else if idtype = 7 then framid = id + 720000;
  keep framid beerperweek9 wineperweek9 liquorperweek9 drinksperweek9 gramsperday9;
run;


/*Gen 2 Exam 10 Omni 1 exam 5 alcohol data extraction*/
data alcohol_10; 
set in1.e_exam_ex10_1b_1409_v1;
  beerperweek10  = K0321;
  wineperweek10 = K0325;
  liquorperweek10 = K0329;
/*Calculate number of drinks and alcohol consumption per week*/
  drinksperweek10 = sum(of beerperweek10 wineperweek10 liquorperweek10);
  gramsperday10 = sum(beerperweek10*13.9 , 15*wineperweek10, 14*liquorperweek10)/7;
/*Create universal identifier - framid */
  if idtype = 1 then framid = id + 80000;
  else if idtype = 7 then framid = id + 720000;
  keep framid beerperweek10 wineperweek10 liquorperweek10 drinksperweek10 gramsperday10;
run;


*Sort all three datasets by using the macro-sort;
%sort(list_sex_17, framid);
%sort(alcohol_1, framid);
%sort(alcohol_2, framid);
%sort(alcohol_3, framid);
%sort(alcohol_4, framid);
%sort(alcohol_5, framid);
%sort(alcohol_6, framid);
%sort(alcohol_7, framid);
%sort(alcohol_8, framid);
%sort(alcohol_omni1_5, framid);
%sort(alcohol_omni1_7, framid);
%sort(alcohol_omni1_8, framid);
%sort(alcohol_9, framid);
%sort(alcohol_10, framid);

/*Merging all alcohol datasets*/
data alcohol_110_17;
merge list_sex_17 (in=inback) alcohol_1-alcohol_8 alcohol_omni1_5 alcohol_omni1_7 alcohol_omni1_8 alcohol_9 alcohol_10;
by framid;
if inback;
/*Creating dichotomous drinking status based on drinks per week*/
array drink{*} drinksperweek1-drinksperweek10; 
array drink_status{*} currdrink1-currdrink10; 
do h = 1 to dim (drink);
if drink(h) = . then drink_status(h) = .; 
else if drink(h) = 0 then drink_status(h) = 0;
else drink_status(h) = 1;  
end;
/*Create drinking history variable*/
array currdrink{*} currdrink2-currdrink10; 
array drinksum{*} currdrink1 drinksum2-drinksum10; 
array drinking_ever{*} drinking_ever2-drinking_ever10; 
do k = 1 to dim(currdrink);
drinksum{k+1} = sum(drinksum{k},currdrink{k});
end;
/*drinking_ever: 0 = never drinker, 1 = current drinker, 2 = ex-drinker*/
do i = 1 to dim(currdrink); 
if currdrink(i) = . then drinking_ever(i) =.; 
else if  currdrink(i) = 1 then drinking_ever(i) =1; 
else if drinksum(i+1) > 0 then drinking_ever(i) = 2; 
else drinking_ever(i) = 0; 
end; 
/*Omni 1 first exam, hence set history of drinking variable to missing*/
if framid > 700000 then drinking_ever5 = .;
/*Creating alcohol category based on alcohol consumption per week*/
array gramsperday{*} gramsperday1-gramsperday10;
array alccat{*} alccat1-alccat10; 
do j = 1 to dim(gramsperday);
/*Alcohol category for men*/
 if sex=1 then do;
        if gramsperday(j) =. then alccat(j)=.;
		  else if gramsperday(j) =0 then alccat(j)=0;
           else if 0<gramsperday(j)<=28 then alccat(j)=1;
		    else if 28<gramsperday(j)<42  then alccat(j)=2;
			  else if gramsperday(j)>=42 then alccat(j)=3;
			  end;
/*Alcohol category for women*/ 
 if sex=2 then do;
		if gramsperday(j) =. then alccat(j)=.;
		  else if gramsperday(j) =0 then alccat(j)=0;
           else if 0<gramsperday(j)<=14 then alccat(j)=1;
		    else if 14<gramsperday(j)<28  then alccat(j)=2;
			  else if gramsperday(j)>=28 then alccat(j)=3;
			  end;
end;
drop h i j k drinksum:;
run;


/*Rename using Gen 2/Omni 1 curated dataset naming convention*/
data alcohol_110_17_1;
retain idtype id framid sex beerperweek1-beerperweek10 wineperweek1-wineperweek5 wineperweek8-wineperweek10 whiteperweek6 whiteperweek7 redperweek6 redperweek7 liquorperweek1-liquorperweek10 drinksperweek1-drinksperweek10 gramsperday1-gramsperday10 currdrink1-currdrink10 drinking_ever2-drinking_ever10 alccat1-alccat10;
set alcohol_110_17;
rename
beerperweek1-beerperweek10=beerperweek_core1-beerperweek_core10
wineperweek1-wineperweek5=wineperweek_core1-wineperweek_core5
wineperweek8-wineperweek10=wineperweek_core8-wineperweek_core10
whiteperweek6=whiteperweek_core6
whiteperweek7=whiteperweek_core7   
redperweek6=redperweek_core6 
redperweek7=redperweek_core7
liquorperweek1-liquorperweek10=liquorperweek_core1-liquorperweek_core10
drinksperweek1-drinksperweek10=drinksperweek_core1-drinksperweek_core10
gramsperday1-gramsperday10=gramsperday_core1-gramsperday_core10
currdrink1-currdrink10=currdrink_core1-currdrink_core10
drinking_ever2-drinking_ever10=drinking_ever_core2-drinking_ever_core10
alccat1-alccat10=alccat_core1-alccat_core10;
drop id idtype sex;
run;
...
set in1.np_summary_through2024_19899;
*Creating universal identifer variables - framid;
if idtype=0 then framid=id;
else if idtype=1 then framid=id+80000;
else if idtype=2 then framid=id+20000;
...
set in1.vr_cancer_2019_a_1162_v2;
*Creating universal identifer variables - framid;
if idtype=0 then framid=id;
else if idtype=1 then framid=id+80000;
else if idtype=2 then framid=id+20000;
...
retain framid first_cancer_date topo_cancer location_cancer hist_cancer multiple_cancer_indicator;
set cancer_all_cohort_1;
*First cancer observation per FHS participant;  
by framid d_date;
if first.framid; 
*1 if participant has more than one row on the earliest date, otherwise 0;
multiple_cancer_indicator = (not last.d_date);
...
keep framid first_cancer_date topo_cancer location_cancer hist_cancer multiple_cancer_indicator;
...
set in1.vr_survdth_2022_a_1537;
*Creating universal identifer variables - framid;
if idtype=0 then framid=id;
else if idtype=1 then framid=id+80000;
else if idtype=2 then framid=id+20000;
...
data mmse_17;
set in1.vr_mmse_ex10_1b_1395_v1;
*Creating universal identifer variables - framid;
if idtype =1 then framid=id+80000;
...
*Framid, testdate, indicator as first three variables;
retain framid testdate CESD_indicator_core8; 
set CESD_exam8_17; 
by framid; 
*1 if this participant has more than one test in original dataset, otherwise 0;
CESD_indicator_core8 = (not last.framid);
...
set in1.vr_educ_2018_a_1307_v1; 
*Creating universal identifer variables - framid;
if idtype = 0 then framid = id; 
...
set in1.neuropath_through2024_269;
*Creating universal identifer variables - framid;
if idtype=0 then framid=id;
else if idtype=1 then framid=id+80000;
else if idtype=2 then framid=id+20000;
...
/*Citation: D_Agostino Sr, R. B., Vasan, R. S., Pencina, M. J., Wolf, P. A., Cobain, M., Massaro, J. M., & Kannel, W. B. (2008). General cardiovascular risk profile for use in primary care: the Framingham Heart Study. Circulation, 117(6), 743-753.*/;
...
/*Citation: Wolf, P. A., D'Agostino, R. B., Belanger, A. J., & Kannel, W. B. (1991). Probability of stroke: a risk profile from the Framingham Study. Stroke, 22(3), 312-318.*/;
...
data out1.curated_bap_17_10272025;
retain idtype id framid sex race_code race ethnicity date_core1-date_core10 date_fu_core10 status_core10 age_core1-age_core10 edu_core2 edu_core8 edu_all;
merge  
backbone_17 (in=in1)
alcohol_110_17_1
NP_cohort17_2
...
*One Gen 2 participant withdrew from the study as of 01/2025; 
if idtype = 1 and id = 1220 then delete; 
if in1;

/*Recategorizing edu_all variable using priority list - refer to coding manual*/
if educg_np2 ne . then edu_all = educg_np2;
else if educg_np1 ne . then edu_all = educg_np1;
run; 



*Check format and accuracy; 
proc print data=out1.curated_bap_17_10272025;
...
data csv_curated_17; 
set out1.curated_bap_17_10272025;
run; 
...
PROC EXPORT DATA= WORK.csv_curated_17
            OUTFILE= "C:\Users\mcum\Desktop\SAS_curated_files\curated_2\Output_files\curated_bap_17_10272025_nolabel.csv" 
...
******************************************************************************************************************************************
Creation of curated dataset (Gen 2 Omni 1) FOR EXTERNAL USE
******************************************************************************************************************************************;

data csv_curated_1_EXT csv_curated_7_EXT ; 
set csv_curated_17; 
if idtype = 1 then output csv_curated_1_EXT; 
else if idtype = 7 then output csv_curated_7_EXT;
run; 

data csv_curated_1_dateEXT;
set csv_curated_1_EXT; 
*Date conversion to days from exam 1;
array date_core[*] date_core:; 
array examdate_np[*] examdate_np:; 
array np_closest_exam_date[*] np_closest_exam_date:; 
array outcome[*] date_fu_core10 normal_date  impairment_date  mild_date  moderate_date  severe_date  eddd  first_chd_date  first_stroke_date  first_af_date  first_cancer_date  last_contact_date  death_date ; 
do i = 2 to dim (date_core); 
date_core(i) = date_core(i) - date_core1;
end; 
do j = 1 to dim (examdate_np); 
examdate_np(j) = examdate_np(j) - date_core1;
np_closest_exam_date(j) = np_closest_exam_date(j) - date_core1;
end; 
do k = 1 to dim (outcome); 
outcome(k) = outcome(k) - date_core1;
end;
date_core1 = 0;  
drop i j k; 
format date_core: examdate_np: np_closest_exam_date: date_fu_core10 normal_date  impairment_date  mild_date  moderate_date  severe_date  eddd  first_chd_date  first_stroke_date  first_af_date  first_cancer_date  last_contact_date  death_date ; 
run; 

data csv_curated_7_dateEXT;
set csv_curated_7_EXT; 
*Date conversion to days from exam 5;
array date_core[*] date_core:; 
array examdate_np[*] examdate_np:; 
array np_closest_exam_date[*] np_closest_exam_date:; 
array outcome[*] date_fu_core10 normal_date  impairment_date  mild_date  moderate_date  severe_date  eddd  first_chd_date  first_stroke_date  first_af_date  first_cancer_date  last_contact_date  death_date ; 
do i = 6 to dim (date_core); 
date_core(i) = date_core(i) - date_core5;
end; 
do j = 1 to dim (examdate_np); 
examdate_np(j) = examdate_np(j) - date_core5;
np_closest_exam_date(j) = np_closest_exam_date(j) - date_core5;
end; 
do k = 1 to dim (outcome); 
outcome(k) = outcome(k) - date_core5;
end;
date_core5 = 0;  
drop i j k; 
format date_core: examdate_np: np_closest_exam_date: date_fu_core10 normal_date  impairment_date  mild_date  moderate_date  severe_date  eddd  first_chd_date  first_stroke_date  first_af_date  first_cancer_date  last_contact_date  death_date ; 
...
data out1.curated_bap_17_10272025_EXT; 
set csv_curated_1_dateEXT csv_curated_7_dateEXT;
array marital_np[*] marital_np:; 
array employment_np[*] employment_np:;
array education_b2_np[*] education_b2_np:;
array occup_b2_np[*] occup_b2_np:;
array age_np[*] age_np:;	
do i = 1 to dim(marital_np); 
if marital_np(i) in (4,5,6) then marital_np(i) = 7;
if employment_np(i) in (2,5,6,7,10,11) then employment_np(i) = 12;
if education_b2_np(i) in (0,1,2,3,4) then education_b2_np(i) = 16; 
if education_b2_np(i) in (5,6) then education_b2_np(i) = 17;
if education_b2_np(i) in (8,9) then education_b2_np(i) = 18;
if occup_b2_np(i) in (5,6,7,8) then occup_b2_np(i) = 11;
if occup_b2_np(i) = 10 then occup_b2_np(i) = .;
if age_np(i) gt 90 then age_np(i) = 90;   /* no age above 90 for age at NP tests */
end; 
array age_core[*] age_core:; 
do j = 1 to dim(age_core); 
if age_core(j) gt 90 then age_core(j) = 90;   /* no age above 90 for age at core exams  */
end; 
drop i j race_code; 
run; 
